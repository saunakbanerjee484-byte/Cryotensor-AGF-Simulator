...
Layout: single-screen [7, 3] column split.
    ...
    - Right (30%): Control Panel -- exact numerical inputs (no sliders) for
                   Sub-Modules 1A (Thermal Matrix Parameters), 1B (Boundary Setup),
                   1C (Phase Engine / run controls).

This file contains ONLY UI wiring. Physics lives in core_physics/, presentation
helpers live in ui_components/.
"""
...
with right:
    ...
    with tab_1c:
        ...
        st.markdown("---")
        run_clicked = st.button("▶ Run Stefan Phase-Change Simulation", type="primary", width="stretch")
        ...
        if run_clicked:
            ...
            with st.spinner("Solving transient Stefan phase-change matrix (implicit, vectorized)..."):
                results = run_stefan_simulation(params, record_every=int(record_every))
            results["critical_cooling_rate"] = critical_cooling_rate
            results["warning_cooling_rate"] = warning_cooling_rate
            st.session_state["results"] = results
            st.success(f"Simulation complete — {len(results['t'])} recorded frames.")
